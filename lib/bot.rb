...
# quelques lignes qui appellent les clés d'API
...
#Ton objectif est de faire un programme qui en prend 5 au hasard, et qui leur tweet une phrase sympa 
#avec le hashtag #bonjour_monde. Un seul tweet par journaliste suffit 

5.times do 
client.update("#{@journalists.sample} waouw merci pour ton travail #bonjour_monde")
end

#Maintenant que tu as dit bonjour à Internet, nous allons te demander de liker les derniers tweets (minimum les 25 derniers) 
...
#Maintenant que tu es la star du like, nous allons te demander de follow les dernières personnes (minimum 20) qui ont tweeté avec le
#hashtag #bonjour_monde.

tweets = client.search("#bonjour_monde", result_type: "recent").take(60).collect do |tweet|
          tweet.user.screen_name.follow
end

=begin
Maintenant tu vas utiliser la fonction Streaming de l'API Twitter afin de réagir en live. Mets en place un robot qui like et 
follow les comptes tweetant le #bonjour_monde dès l'apparition du Tweet.
...
def login_twitter (login_twitter)
  return client not nil
end
=end